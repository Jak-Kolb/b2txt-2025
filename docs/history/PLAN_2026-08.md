...
| `smooth_lookahead` is a clean causality knob | **Confounded** — and now **deconfounded by R-D1a (§7.1, 2026-08-09)**: bandwidth −0.200 pts, causality **+0.030** (under the 0.041 floor). L=4→L=0 removes 80 ms lookahead *and* narrows the low-pass by 37 % (σ_eff 1.852→1.161 bins) *and* raises peak weight 66 %; the narrowing is what did the work. [AUDIT F2] |
...
1. **The bandwidth-matched control is `smooth_kernel_std: 3.4` at `lookahead: 0`** (σ_eff 1.899,
   ...
   > **CORRECTED 2026-08-08, before the run: use `smooth_kernel_std: 3.24`, not 3.4.** Re-deriving
   > the table from `data_augmentations.py:17-27` at 0.02 resolution shows 3.4 is *not* the nearest
   > reachable value — it overshoots by **0.0478** bins, while **3.24 gives σ_eff 1.8534 against the
   > 1.8516 target, a residual of 0.0019 — 26× closer, at the identical 8 taps and the same latency
   > cost.** Since this run's only deliverable is a *bandwidth-matched* control, a 2.6 % bandwidth
   > mismatch is exactly the thing a reviewer would attack if the result came out ambiguous. The
   > coarse 0.1-step grid in the table above is what hid this.
...
|---|---|---|---|
| **R-D1a** (control) | **`smooth_kernel_std: 3.24`**, `lookahead: 0` | PER rises toward **10.21** — the L=0 win was bandwidth | PER **falls** below 10.04 |
| **R-D1b** (lever) | `smooth_kernel_std: 1.5`, `lookahead: 0` | PER **falls** below 10.04 | PER rises |

#### R-D1a decision gate — PRE-REGISTERED 2026-08-08

**Config.** `causal_la0` anchor config (Group 3 was NEUTRAL and is not carried, §6.4), changing
**one variable**: `smooth_kernel_std: 2 → 3.24`. `smooth_lookahead: 0`, `seed: 10`, 120k batches.
Validity runs are never bundled (§0) and this one is single-variable by construction.

**Primary metric: val PER (best and last-10 mean)**, against the two anchors this control sits
between — `causal_la0` **10.040 / 10.087** (σ_eff 1.161) and `causal_la4` **10.21** (σ_eff 1.852).
This is the one gate in the project that is *correctly* PER-based: the deliverable is a comparison
against two PER anchors, not a deployment decision.

**Report the outcome as a fraction of the gap explained**, not as a binary:

```
f = (PER_RD1a - 10.040) / (10.21 - 10.040)
```

| f | Reading |
|---|---|
| **f ≳ 0.75** (PER ≳ 10.17) | **Bandwidth explains the L=0 advantage.** `AUDIT F2` confirmed, `RESEARCH_PROPOSALS §1` falsified. `smooth_kernel_std` becomes a real tuning lever with a known direction (narrower). Run R-D1b. |
| **f ≲ 0.25** (PER ≲ 10.08) | **Bandwidth does not explain it** — something about the lookahead axis itself is doing the work, and the causality claim survives a serious attempt to kill it. R-D1b can be skipped. |
| between | Partial. Report the decomposition honestly; do not round to either story. |

**Resolution limit, stated up front:** the 0.041-pt replicate floor over a 0.17-pt gap means **f is
resolvable to only about ±0.24**. This run can distinguish "most of it" from "little of it" and
cannot do better. Do not report f to two decimals.

**Secondary, recorded but not gating:** val-dev WER at a re-swept decode config (the posterior
changes, so the `acoustic_scale` optimum will move), plus entropy / p(blank)>0.999 — a 65 % wider
kernel should *soften* the posterior, and §6.4 showed softening is worth ~0.1 WER pts at most.

**Both outcomes are publishable and the run cannot be killed** — that is what makes it a control.
It is also the only remaining item that addresses the project's *central* question rather than its
accuracy.

#### R-D1a RESULT 2026-08-09 — the confound is resolved, and BOTH halves are real

334.79 min, 120k batches, `trained_models/rd1a_bwmatched`. Single variable
(`smooth_kernel_std: 2 → 3.24`), evaluated against the gate exactly as pre-registered.

| | σ_eff | lookahead | val PER (best) | val PER (last-10) | val CTC |
|---|---|---|---|---|---|
| `causal_la0` | 1.161 | 0 ms | **10.040 %** | 10.087 % | 21.79 |
| `causal_la4` | 1.852 | 80 ms | **10.210 %** | 10.250 % | 22.58 |
| **R-D1a** | **1.853** | **0 ms** | **10.240 %** | 10.281 % | 22.47 |

**f = 1.18 (best) / 1.19 (last-10) — far above the 0.75 threshold. `AUDIT F2` is CONFIRMED and
`RESEARCH_PROPOSALS §1` is FALSIFIED.** The two estimates agreeing to 0.01 is a good sign the
number is not an artifact of checkpoint selection.

**The decomposition closes exactly, which is the strongest evidence the control is well-posed:**

```
observed L=4 -> L=0        10.210 -> 10.040   = -0.170 pts
  bandwidth   (sigma_eff 1.853 -> 1.161 at lookahead=0)   -0.200   <- 4.9x the noise floor, REAL
  causality   (lookahead 80 ms -> 0 ms at sigma_eff ~1.85) +0.030   <- BELOW the 0.041 floor, FREE
                                                          -------
                                                          -0.170   exact
```

**Two findings, and the second is the one the project was built to get:**

1. **The 0.17-pt "causality gain" was bandwidth, not causality.** Truncating the kernel to make it
   causal *also* narrowed it by 37 %, and the narrowing is what did the work. Every previous
   statement that "L=0 beats L=4" was measuring a low-pass filter, not a latency property.
2. **Causality is genuinely free — but for a reason nobody had established.** At *matched*
   bandwidth, removing 80 ms of lookahead costs **+0.030 pts**, under the 0.041-pt reproducibility
   floor. The old claim was accidentally right for the wrong reason; it is now right for a measured
   one. **This is the project's central question, answered with a proper control.**

**Consequence: `smooth_kernel_std` is a real tuning lever and its direction is narrower.** Going
1.853 → 1.161 bought 0.200 pts at zero latency cost (the tail is all past). R-D1b (σ=1.5,
σ_eff 0.902) tests whether the trend continues or has already passed its optimum — and per the gate,
**f ≳ 0.75 means R-D1b is worth the run.**

**A secondary prediction FAILED, and it is recorded as such.** The gate predicted a 60 % wider
kernel would *soften* the posterior. It did not: entropy **0.0324 → 0.0320 nats**, p(blank)>0.999
**71.54 % → 71.88 %** — flat to slightly sharper. Posterior sharpness on this model appears
insensitive to input smoothing bandwidth, which removes smoothing from the list of levers that
could address the C10/C20 calibration hypothesis.

##### The WER side: the bandwidth effect AMPLIFIES 3× from PER to WER

Decode re-swept per model, as required. The optimum moved **0.4 → 0.30** (`blank_penalty` 30), and
the extended grid confirms a genuine interior minimum (0.25 → 8.52 %, 0.20 → 9.76 %, 0.15 → 11.73 %
on the 400-trial probe), not a boundary artifact.

| | val PER | WER (all) | **WER (unseen)** |
|---|---|---|---|
| `causal_la0` (σ_eff 1.161, ac 0.4) | 10.040 % | 8.49 % | **8.72 %** |
| R-D1a (σ_eff 1.853, ac 0.30) | 10.240 % | 9.07 % | **9.33 %** |
| Δ | **+0.200** | **+0.58** | **+0.61** |

**A 0.20-pt PER penalty becomes a 0.61-pt WER penalty — a 3.0× amplification.** Two consequences:

1. **This is the first acoustic-side lever in the project with a positive, measured WER mechanism.**
   Every other one measured neutral or negative (C10 −0.05, C16 +0.46/+0.69, G2 +0.61 inverted,
   G3a −0.06). Narrowing σ_eff 1.853 → 1.161 is worth **0.61 WER pts at zero latency cost**, because
   the entire kernel tail is in the past.
2. **It is not the G2 pattern.** G2 moved PER and WER in *opposite* directions; bandwidth moves them
   in the *same* direction and amplifies. So for this lever — unlike masking — PER is a valid (if
   understated) proxy. Do not generalize either way: the two behave differently and both were
   measured.

**This materially strengthens the case for R-D1b**, which now has a WER rationale rather than only a
PER one: if 1.853 → 1.161 buys 0.61 WER pts, the open question is whether 1.161 → 0.902 continues
the trend or has already passed the optimum. Given §1.13 concluded "the remaining reachable error is
acoustic", this is the one acoustic knob known to move it.

*(Operational note: this run took three launches. The first died at batch 83,000 from GPU
contention when a game was started; the second died 35 s in from an unexplained transient
`cudaErrorUnknown`. A 4-minute stress test reproducing the exact workload — 5-layer GRU, batch 64,
bf16, `ctc_loss`, 7.59 GB peak — passed cleanly, so neither crash is attributable to this code.
`model_training/run_with_watchdog.sh` now auto-restarts **only** on crashes inside 30 minutes, where
restarting from scratch is free and keeps the run uninterrupted; a late crash is preserved and
escalated instead. The successful run completed on attempt 1.)*

Artifacts: `results/rd1a_logits.npz`, `results/rd1a_sweep*.json`,
`model_training/trained_models/rd1a_bwmatched/`, `model_training/rd1a_bwmatched.log`,
`model_training/trained_models/_crashed_rd1a_83k/` (the contention crash, kept as the partial that
first suggested this result).

**R-D1a is done.** R-D1b is licensed by the gate (f ≳ 0.75) and now has a WER rationale, but it
is deferred: the paper is systems-and-measurement (2026-08-18), `val-test` is spent, and a new
stack cannot be held out. Revisit only as an appendix after R-D2/R-D3 land. The wider-kernel ring
buffer (8 taps at σ=3.24 vs 5 at σ=2) is not needed — the lever's direction is narrower.
...
*Seed count in the table above is superseded by the 2026-08-18 gate below (two seeds planned;
split → third). Thresholds unchanged. Void is already closed: G1-d passed at 3.9e-05.*

#### R-D2 / R-D3 decision gate — PRE-REGISTERED 2026-08-18, unlaunched

The paper is systems-and-measurement. This pair is the last experiment that can still invalidate
the claim "end-to-end causal." R-D1a made the *smoother* causal at matched bandwidth; every number
in this file still sits on whole-block z-scored inputs `[AUDIT F1]`. Both outcomes below are
publishable. Do not skip the pair because the expected PER move is "a cost."

**Variance floor — DECIDED here, not after the run.** C8 (`causal_normalize.py:37-40, 122-157`)
left this open: 0.0175 % of `(t, channel)` emissions have running std below 1e-3 because sparse
threshold-crossing channels go locally constant, and no fixed epsilon is scale-invariant.

| Choice | What it does | Why not |
|---|---|---|
| `_EPS = 1e-8` only (the identity-checked divider) | Live mass unchanged; floor-dominated mass can take \|z\| ≫ 10 | C8 *excluded* that mass from the residual rather than endorsing those values as training inputs |
| Larger `_EPS` on every channel (e.g. 1e-4) | Shrinks the live z-scores too | G1-d would no longer apply to the thing we train; would need a re-check |
| Percentile of observed variance | Data-dependent | Deciding the floor by looking at the run is deciding after |

**The floor is: emit 0 when running std < 1e-3; otherwise divide with `_EPS = 1e-8`.** Same
threshold C8 used to mark an emission floor-dominated (`--min_std 1e-3`). Emitting 0 is already
what warmup does (`causal_normalize.py:71-72`), so it is one rule — do not normalize until the
estimator is usable. Affine invariance still holds on both sides of the cut (0 = 0). The live
99.9825 % is the function G1-d passed. 0.0175 % is too rare to regularize anything and too
dangerous as a 1e4-scale tail.

Record as a limitation, next to the per-trial reset and the `(mean, var) = (0, 1)` init: this
floor is a design choice, not a reconstruction of the unreleased raw-feature pipeline.

**Config.** `causal_la0` anchor (Group 3 not carried, §6.4; R-D1b deferred). Single variable:
turn causal rolling normalization on. Two seeds, two runs, never bundled.

```yaml
# rnn_args.yaml — the only new keys. Everything else matches causal_la0.
output_dir: trained_models/rd2_causalnorm_s10      # R-D3: rd3_causalnorm_s11
checkpoint_dir: trained_models/rd2_causalnorm_s10/checkpoint
seed: 10                                           # R-D3: 11. Never touch dataset.seed.
dataset:
  causal_normalize: true
  causal_norm_half_life_bins: 500                  # 10 s at 20 ms
  causal_norm_warmup_bins: 50                      # 1 s; inside the 3,380 ms pre-speech period
  causal_norm_min_std: 0.001                       # THE floor. Emit 0 below this.
```

`_EPS = 1e-8` stays a numerical constant in `causal_normalize.py`, not a swept hyperparameter.

Apply in `dataset.py::BrainToTextDataset.__getitem__` immediately after `input_features` is read
at `:130`, before `pad_sequence` at `:149`. The streaming path (`benchmark/common.py`,
`streaming_infer.py`) must apply the same function before `smooth_features` / `_append_raw`, or
the equivalence gate is not a gate.

**Primary metric: val PER (best and last-10 mean)** vs `causal_la0` **10.040 / 10.087**. This is a
validity comparison against a PER anchor, same justification as R-D1a. Secondary, recorded but
not gating: val-dev WER at a re-swept decode config (the posterior will change), plus the §6.4
M1 split (prefers-correct / tied / prefers-wrong) so a PER move can be read as acoustic
discrimination or as another G2-style sharpening.

Two seeds are planned (R-D2 = 10, R-D3 = 11). The table above said "2 of 3" when a third seed
was imagined; with two runs the rule is:

| Outcome | Rule |
|---|---|
| **H2** | both seeds rise ≥ **0.20** PER pts (~5× the 0.041 floor) |
| **H2-null** | both seeds move < 0.20 pts. Licenses every prior number as real-time-valid |
| **Split** | one seed each side of 0.20. Do not call it. Run a third seed before writing the sentence |
| **Void** | already closed — G1-d passed at 3.9e-05 |

**Do not convert a PER-side H2 into a WER reject, or a PER-side H2-null into a WER celebration,
without the re-swept WER.** G2 taught that. Report both.

**What each outcome does to the rest of Group 4:**

- **H2** → the price of end-to-end causality is that number. The deployed config *is* the
  causal-norm model. R-D2+R-D3 are already its two seeds; **C27 is done.** Equivalence gate and
  the latency ledger re-run on this model. R-D1b, if ever run, is on this config, not on `causal_la0`.
- **H2-null** → every number in this file stands. Deployed config stays `causal_la0`. **C27
  becomes the next run** (seed 11 of that config) so the paper has a WER error bar.
- **Split** → third seed, then the same two bullets.

Smoke (2,000 batches) is mandatory and must have `causal_normalize: true`, or it will not
exercise the path. No launch without an explicit go.

**R-D4 → C27 — seed replication, gated on H2-null.** Re-run the *winning* config at seed 11 (vary
**only** the torch/numpy seed at `rnn_args.yaml:48`, never `dataset.seed` at `:81`, or the split
changes). If H2 fired, R-D3 already is that run. The headline currently has n=1 and no WER error
bar; the 0.041-pt figure is PER only.

### 7.2 The LM/stack work (0 GPU training)

**Status 2026-08-18.** C28 (4-gram) and C29 (gpt2-large n-best rescoring) are **done** — they are
the evaluated stack, §1.12–1.14. **C29b shallow fusion must not be built as specified** (§1.13).
C30 (endpointing) is on-theme for the systems paper and still unbuilt. C31 (adaptive compute) has
an unbounded worst case; skip unless the latency ledger has a hole only it fills. The C28/C29
text below is the original spec, left as the record of what was planned, not a to-do.
...
| **G3a** | `trained_models/g3_objective` | C22+C23 (C20 dropped, C21 unbuilt, C24 rejected) | **9.922** | **9.984** | 18.22 | **8.38 / 8.66 unseen** | — | — | 338 | **NEUTRAL, not carried** — see §6.4 |
| **R-D1a** | `trained_models/rd1a_bwmatched` | C25: `smooth_kernel_std` 2→3.24, L=0 | **10.240** | **10.281** | 22.47 | **9.07 / 9.33 unseen** | — | — | 335 | **AUDIT F2 CONFIRMED** — bandwidth −0.200, causality +0.030 (free). See §7.1 |
...
| ~~Q2~~ | ~~Constrained-baseline WER~~ | — | **MEASURED: 42.42 % (1-gram defaults) → 35.51 % swept → 8.49 % 4-gram** |
| Q3 | Which LM produced 2.66 % | how much is reachable incrementally | G0.5 |
| ~~Q6~~ | ~~Is the L=0 advantage causality or bandwidth?~~ | — | **RESOLVED 2026-08-09 by R-D1a: bandwidth −0.200, causality +0.030 (under the 0.041 floor). Free.** |
| Q7 | Does the la0-vs-la4 difference survive replication? | error bar on the headline | R-D4 / C27 — only if H2-null; else R-D2+R-D3 are the two seeds |
| ~~Q8~~ | ~~Is the patch embedding phase-sensitive?~~ | — | **G1-c PASS: 0.091-pt spread vs a 0.5-pt gate (§1.7)** |
| ~~Q9~~ | ~~Can a 4-gram be pruned under 32 GB without destroying it?~~ | — | **YES: TLG 3.94 GB at 15.0 GB peak (C28)** |
